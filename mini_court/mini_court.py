...
sys.path.append("../")
import constants
from utils import (convert_pixel_distance_to_meters, convert_meters_to_pixel_distances ,get_foot_position,get_height_of_bbox,
get_closest_keypoint_index, measure_xy_distance,get_center_of_bbox, measure_distance)
...
class MiniCourt():
    ...
    def convert_meters_to_pixels(self, meters):
        return convert_meters_to_pixel_distances(meters, constants.DOUBLE_LINE_WIDTH, self.court_drawing_width)
    
    def set_court_drawing_keypoints(self):
        drawing_key_points = [0] * 28  # create a list of zeros of  28 keyponits
    ...
    def set_court_lines(self):
        self.lines = [
            ...
            (8,9),
            (10,11),
            (2,3)  # Fixed: removed duplicate (10,11)
        ]
    ...
    def set_canvas_background_box_position(self, frame):
        ...
        self.end_x = frame.shape[1] - self.buffer  # start from end
    ...
    def draw_background_rectangle(self, frame):
        # create a dummy shapes array for transparency
        shapes = np.zeros_like(frame ,np.uint8)
        # draw rectangle on the dummy shapes
        cv.rectangle(shapes, (self.start_x, self.start_y), (self.end_x, self.end_y), (255, 255, 255), -1)
        out = frame.copy()
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_key_points):
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            # Safe ball box extraction with error handling
            ball_box = None
            if (frame_num < len(ball_boxes) and 
                ball_boxes[frame_num] and 
                isinstance(ball_boxes[frame_num], dict) and 
                1 in ball_boxes[frame_num]):
            ...
            # Process ball separately (FIXED: Process ball for every frame independently)
            ball_dict = {}
            if ball_box is not None:
                ball_position = get_center_of_bbox(ball_box)
                
                # Find closest player to use their height for reference
                if player_bbox:
                    closest_player_id = min(player_bbox.keys(), 
                                          key=lambda x: measure_distance(ball_position, get_center_of_bbox(player_bbox[x])))
                    
                    # Get The closest keypoint for ball
                    closest_key_point_index = get_closest_keypoint_index(ball_position, original_court_key_points, [0,2,12,13])
                    closest_key_point = (original_court_key_points[closest_key_point_index*2], 
                                        original_court_key_points[closest_key_point_index*2+1])
                    
                    # Use closest player's height data for ball position calculation
                    frame_index_min = max(0, frame_num-20)
                    frame_index_max = min(len(player_boxes), frame_num+50)
                    bboxes_heights_in_pixels = [get_height_of_bbox(player_boxes[i][closest_player_id]) for i in range(frame_index_min, frame_index_max) if closest_player_id in player_boxes[i]]
                    max_player_height_in_pixels = max(bboxes_heights_in_pixels) if bboxes_heights_in_pixels else 100
                    ...
                    mini_court_ball_position = self.get_mini_court_coordinates(ball_position,
                                                                            ...
                                                                            closest_key_point_index, 
                                                                            max_player_height_in_pixels,
                                                                            player_heights[closest_player_id]
                                                                            )
                    ball_dict[1] = mini_court_ball_position
            
            output_ball_boxes.append(ball_dict)

        return output_player_boxes, output_ball_boxes
    
    def draw_points_on_mini_court(self, frames, positions, color=(0,255,0)):
        for frame_num, frame in enumerate(frames):
            if frame_num < len(positions):  # Safety check
